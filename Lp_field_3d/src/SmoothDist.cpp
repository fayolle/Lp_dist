...
void printHelp() {
  ...
  cout << "Syntax:" << endl;
  cout << "\t./smoothdist inputmesh outputvtk xm ym zm xM yM zM nx ny nz order normalized?" << endl;
  cout << "where:" << endl;
...
int main(int argc, char** argv) {
  ...
  if (isNormalized) {
    cout << "Compute normalized psi" << endl;
    results = computeNormalizedPsi(structuredGrid, order, triMesh);
  } else {
    cout << "Compute non-normalized psi" << endl;
    results = computePsi(structuredGrid, order, triMesh);
  }
